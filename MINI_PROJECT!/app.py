...
def proceed_to_payment_hosteller():
    ...
    try:
        ...
        # For hosteller, we set a fixed payable amount, e.g., 1500
        session["total_amount"] = 150
...
def process_payment():
    # Retrieve form data from payment.html
    phone = request.form.get("phone")
    amount = request.form.get("amount")
    # Here, add any payment processing logic (e.g. updating a database, confirming payment, etc.)
    # Optionally, store information in session if needed:
    session['phone'] = phone
    session['paid_amount'] = amount
    # After processing, redirect the user to the ticket page.
...
def ticket():
    ...
    register_number = session["user"]  # Get register number from session

    try:
        cursor.execute("SELECT register_number, full_name, email FROM students WHERE register_number = %s", (register_number,))
        student = cursor.fetchone()

        if not student:
            flash("Error: Student details not found!", "error")
            return redirect(url_for("payment"))
        ...
        student_data = {
            ...
            "full_name": student[1],
            "email": student[2],
            "paid_amount": session.get("paid_amount", "N/A"),
        }

        return render_template('ticket.html', student=student_data)
